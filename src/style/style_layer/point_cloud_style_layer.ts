...
import {PointCloudBucket} from '../../data/bucket/point_cloud_bucket';
import {polygonIntersectsBufferedPoint} from '../../util/intersection_tests';
import {getMaximumPaintValue, translateDistance, translate} from '../query_utils';
import properties, {CircleLayoutPropsPossiblyEvaluated, CirclePaintPropsPossiblyEvaluated} from './point_cloud_style_layer_properties.g';
...
import type {CircleLayoutProps, CirclePaintProps} from './point_cloud_style_layer_properties.g';
import type {VectorTileFeature} from '@mapbox/vector-tile';

/**
 * A style layer that defines a circle
 */
export class PointCloudStyleLayer extends StyleLayer {
    ...
    createBucket(parameters: BucketParameters<any>) {
        return new PointCloudBucket(parameters);
    }

    queryRadius(bucket: Bucket): number {
        const pointCloudBucket: PointCloudBucket<PointCloudStyleLayer> = (bucket as any);
        return getMaximumPaintValue('circle-radius', this, pointCloudBucket) +
            getMaximumPaintValue('circle-stroke-width', this, pointCloudBucket) +
